...
const PollingPanel = () => {
  ...
  useEffect(() => {
    ...
    socket.on("vote-cast", ({ voter_id }) => {
      setVoters((prev) =>
        prev.map((v) => (v._id === voter_id ? { ...v, hasVoted: true } : v))
      );
  ...
  const handleCancelVote = async () => {
    ...
    try {
      ...
      alert(
        "Error cancelling vote: " + (err.response?.data?.error || err.message)
      );
    }
  };
  ...
  return (
    <div className="container mt-4">
      <div className="d-flex justify-content-between align-items-center mt-3 mb-3">
        <h4>
          <i className="fas fa-poll me-2"></i>Polling Panel
        </h4>
        <button
          ...
          {pendingVoter && (
            <div className="mt-3">
              <div className="alert alert-warning">
                <strong>{pendingVoter.name}</strong> ({pendingVoter.roll_number}
                ) is currently authorized to vote.
              </div>
